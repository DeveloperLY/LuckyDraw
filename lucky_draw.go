...
import (
	...
	"log"
	mysqlWrapper "luckydraw/mysql"
	redisWrapper "luckydraw/redis"
	"math/rand"
	"strconv"
	"time"
)
...
func GetAllAwardBatch() []AwardBatch {
	...
	for index, awardBatch := range awardBatches {
		awardBatches[index].SetTotalAmount(Config.AwardMap[awardBatch.GetName()])
	}
...
func GetAwardBatch() *AwardBatch {
	...
	// 判断是否到达奖品释放时间点
	startTime , _ := ParseStringToTime(Config.Award.StartTime)
	endTime , _ := ParseStringToTime(Config.Award.EndTime)
...
func InitAwardPool() {
	...
	conn.Send("HSET", getAwardBalanceKey(), "A", Config.AwardMap["A"])
	conn.Send("HSET", getAwardBalanceKey(), "B", Config.AwardMap["B"])
	conn.Send("HSET", getAwardBalanceKey(), "C", Config.AwardMap["C"])
	conn.Flush()
